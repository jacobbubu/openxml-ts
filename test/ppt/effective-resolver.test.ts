...
function rewriteThemeXml(part: { writeSync(s: string): void }, color: string): void {
  part.writeSync(
    `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>` +
      `<a:theme xmlns:a="${A}" name="TestTheme">` +
      `<a:themeElements>` +
      `<a:clrScheme name="Test"><a:dk1><a:srgbClr val="${color}"/></a:dk1>` +
      `<a:lt1><a:srgbClr val="FFFFFF"/></a:lt1><a:dk2><a:srgbClr val="000000"/></a:dk2>` +
      `<a:lt2><a:srgbClr val="EEEEEE"/></a:lt2><a:accent1><a:srgbClr val="${color}"/></a:accent1>` +
      `<a:accent2><a:srgbClr val="${color}"/></a:accent2><a:accent3><a:srgbClr val="${color}"/></a:accent3>` +
      `<a:accent4><a:srgbClr val="${color}"/></a:accent4><a:accent5><a:srgbClr val="${color}"/></a:accent5>` +
      `<a:accent6><a:srgbClr val="${color}"/></a:accent6><a:hlink><a:srgbClr val="${color}"/></a:hlink>` +
      `<a:folHlink><a:srgbClr val="${color}"/></a:folHlink></a:clrScheme>` +
      `<a:fontScheme name="Test"><a:majorFont><a:latin typeface="Marker-${color}"/><a:ea typeface=""/><a:cs typeface=""/></a:majorFont>` +
      `<a:minorFont><a:latin typeface="Marker-${color}"/><a:ea typeface=""/><a:cs typeface=""/></a:minorFont></a:fontScheme>` +
...
describe("EffectiveResolver · 链路缺失场景", () => {
  ...
  it("Master 无 theme 关系 → effective* 全 undefined", async () => {
    const doc = PresentationDocument.create();
    const masterPart =
      doc.presentationPart!.slideParts[0]!.slideLayoutPart!.slideMasterPart!.part;
