...
// @see DocumentFormat.OpenXml.Spreadsheetml2014Revision.RevListAutoExpandRw
 ...
 *
 * Element: `xr:xrrListExpR` */
export class RevListAutoExpandRw extends OpenXmlLeafElement {
  override readonly localName = "xrrListExpR" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      ...
      case "refAdded": this.refAdded = StringValue.parse(value); return;
      case "listGuid": this.listGuid = StringValue.parse(value); return;
    }
  ...
  protected override collectAttributes(): [string, string][] {
    ...
    if (this.refAdded !== undefined) out.push(["refAdded", this.refAdded.toString()]);
    if (this.listGuid !== undefined) out.push(["listGuid", this.listGuid.toString()]);
    return out;
  }

  /** 校验所有 RequiredValidator 标注的属性都存在；缺失抛 REQUIRED_ATTR_MISSING。 */
  validateRequired(): void {
    assertRequired(this.rev, { attribute: ":rev", elementClass: "RevListAutoExpandRw" });
    assertRequired(this.uid, { attribute: ":uid", elementClass: "RevListAutoExpandRw" });
    assertRequired(this.sh, { attribute: ":sh", elementClass: "RevListAutoExpandRw" });
    assertRequired(this.refAdded, { attribute: ":refAdded", elementClass: "RevListAutoExpandRw" });
    assertRequired(this.listGuid, { attribute: ":listGuid", elementClass: "RevListAutoExpandRw" });
  }
}
