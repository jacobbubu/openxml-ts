// THIS FILE IS GENERATED. DO NOT EDIT.
// Source: /Users/rongshen/github/Open-XML-SDK/data/schemas/schemas_microsoft_com_office_spreadsheetml_2022_featurepropertybag.json
// @see DocumentFormat.OpenXml.Spreadsheetml2022Featurepropertybag.TotalsRowBorderRevDxfTableRevDxf

import {
  OpenXmlCompositeElement,
  OpenXmlElementList,
} from "../../../element/index.js";

/** Defines the TotalsRowBorderRevDxfTableRevDxf Class.
 *
 * Element: `xfpb:totalsRowBorderRevDxf` */
export class TotalsRowBorderRevDxfTableRevDxf extends OpenXmlCompositeElement {
  override readonly localName = "totalsRowBorderRevDxf" as const;
  override readonly prefix = "xfpb" as const;
  override readonly namespaceUri = "http://schemas.microsoft.com/office/spreadsheetml/2022/featurepropertybag" as const;
  override readonly children: OpenXmlElementList = new OpenXmlElementList(this);



}
