// THIS FILE IS GENERATED. DO NOT EDIT.
// Source: /Users/rongshen/github/Open-XML-SDK/data/schemas/schemas_microsoft_com_office_word_2006_wordml.json
// @see DocumentFormat.OpenXml.Word2006Wordml.MismatchedKeyMapCustomization

import {
  OpenXmlCompositeElement,
  OpenXmlElementList,
} from "../../../element/index.js";

/** Defines the MismatchedKeyMapCustomization Class.
 *
 * Element: `wne:keymapsBad` */
export class MismatchedKeyMapCustomization extends OpenXmlCompositeElement {
  override readonly localName = "keymapsBad" as const;
  override readonly prefix = "wne" as const;
  override readonly namespaceUri = "http://schemas.microsoft.com/office/word/2006/wordml" as const;
  override readonly children: OpenXmlElementList = new OpenXmlElementList(this);



}
