...
// @see DocumentFormat.OpenXml.200304Emma.Group
...
export class Group extends OpenXmlCompositeElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      ...
      case "emma:confidence": this.confidence = DecimalValue.parse(value); assertNumber(this.confidence, { min: 0, max: 1 }, { attribute: "emma:confidence", elementClass: "Group" }); return;
      ...
      case "emma:cost": this.cost = DecimalValue.parse(value); assertNumber(this.cost, { min: 0, max: 10000000 }, { attribute: "emma:cost", elementClass: "Group" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.id, { attribute: ":id", elementClass: "Group" });
  }
}
