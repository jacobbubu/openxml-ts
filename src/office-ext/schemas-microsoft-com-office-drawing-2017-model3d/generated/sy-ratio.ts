// THIS FILE IS GENERATED. DO NOT EDIT.
// Source: /Users/rongshen/github/Open-XML-SDK/data/schemas/schemas_microsoft_com_office_drawing_2017_model3d.json
// @see DocumentFormat.OpenXml.Drawing2017Model3d.SyRatio

import {
  Int32Value,
  OpenXmlLeafElement,
  assertRequired,
} from "../../../element/index.js";

/** Defines the SyRatio Class.
 *
 * Element: `am3d:sy` */
export class SyRatio extends OpenXmlLeafElement {
  override readonly localName = "sy" as const;
  override readonly prefix = "am3d" as const;
  override readonly namespaceUri = "http://schemas.microsoft.com/office/drawing/2017/model3d" as const;


  /** Numerator (:n) */
  numerator: Int32Value | undefined;

  /** Denominator (:d) */
  denominator: Int32Value | undefined;

  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "n": this.numerator = Int32Value.parse(value); return;
      case "d": this.denominator = Int32Value.parse(value); return;
    }
    super.applyAttribute(qname, value);
  }

  protected override collectAttributes(): [string, string][] {
    const out: [string, string][] = [];
    for (const [k, v] of this.extendedAttributes) out.push([k, v]);
    if (this.numerator !== undefined) out.push(["n", this.numerator.toString()]);
    if (this.denominator !== undefined) out.push(["d", this.denominator.toString()]);
    return out;
  }

  /** 校验所有 RequiredValidator 标注的属性都存在；缺失抛 REQUIRED_ATTR_MISSING。 */
  validateRequired(): void {
    assertRequired(this.numerator, { attribute: ":n", elementClass: "SyRatio" });
    assertRequired(this.denominator, { attribute: ":d", elementClass: "SyRatio" });
  }
}
