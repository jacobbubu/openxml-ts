...
export class SpreadsheetDocument {
  ...
  /**
   * 在 Excel 文档包内创建一个媒体 Data Part（MediaDataPart）并把引用关系挂到
   * workbookPart（Epic-128）。
   *
  ...
  addMediaDataPart(
    ...
    const wb = this.workbookPart;
    if (wb === undefined) {
      throw new OpenXmlPackageError({
        code: "PART_NOT_FOUND",
        message: "addMediaDataPart: workbookPart is missing",
      });
    }
  ...
  /**
   * 在 Excel 文档包内创建视频 MediaDataPart 并把 VideoReferenceRelationship 挂到
   * workbookPart（Epic-128）。
   */
  ...
  /**
   * 在 Excel 文档包内创建音频 MediaDataPart 并把 AudioReferenceRelationship 挂到
   * workbookPart（Epic-128）。
   */
  ...
  /**
   * 在 Excel 文档包内创建媒体 MediaDataPart 并把 MediaReferenceRelationship 挂到
   * workbookPart（Epic-128）。
   */
