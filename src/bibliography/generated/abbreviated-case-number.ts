// THIS FILE IS GENERATED. DO NOT EDIT.
// Source: /Users/rongshen/github/Open-XML-SDK/data/schemas/schemas_openxmlformats_org_officeDocument_2006_bibliography.json
// @see DocumentFormat.OpenXml.Bibliography.AbbreviatedCaseNumber

import {
  OpenXmlElementList,
  OpenXmlLeafElement,
} from "../../element/index.js";

/** Abbreviated Case Number.
 *
 * Element: `b:AbbreviatedCaseNumber` */
export class AbbreviatedCaseNumber extends OpenXmlLeafElement {
  override readonly localName = "AbbreviatedCaseNumber" as const;
  override readonly prefix = "b" as const;
  override readonly namespaceUri = "http://schemas.openxmlformats.org/officeDocument/2006/bibliography" as const;




}
