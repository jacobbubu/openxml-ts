// THIS FILE IS GENERATED. DO NOT EDIT.
// Source: /Users/rongshen/github/Open-XML-SDK/data/schemas/schemas_microsoft_com_office_spreadsheetml_2009_9_main.json
// @see DocumentFormat.OpenXml.Excel2009.OlapSlicerCacheLevelData

import {
  OpenXmlCompositeElement,
  OpenXmlElementList,
  StringValue,
  UInt32Value,
  assertRequired,
} from "../../element/index.js";

/** Defines the OlapSlicerCacheLevelData Class.
 *
 * Element: `x14:level` */
export class OlapSlicerCacheLevelData extends OpenXmlCompositeElement {
  override readonly localName = "level" as const;
  override readonly prefix = "x14" as const;
  override readonly namespaceUri = "http://schemas.microsoft.com/office/spreadsheetml/2009/9/main" as const;
  override readonly children: OpenXmlElementList = new OpenXmlElementList(this);

  /** uniqueName (:uniqueName) */
  uniqueName: StringValue | undefined;

  /** count (:count) */
  count: UInt32Value | undefined;

  /** sortOrder (:sortOrder) */
  sortOrder: StringValue | undefined;

  /** crossFilter (:crossFilter) */
  crossFilter: StringValue | undefined;

  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "uniqueName": this.uniqueName = StringValue.parse(value); return;
      case "count": this.count = UInt32Value.parse(value); return;
      case "sortOrder": this.sortOrder = StringValue.parse(value); return;
      case "crossFilter": this.crossFilter = StringValue.parse(value); return;
    }
    super.applyAttribute(qname, value);
  }

  protected override collectAttributes(): Array<[string, string]> {
    const out: Array<[string, string]> = [];
    for (const [k, v] of this.extendedAttributes) out.push([k, v]);
    if (this.uniqueName !== undefined) out.push(["uniqueName", this.uniqueName.toString()]);
    if (this.count !== undefined) out.push(["count", this.count.toString()]);
    if (this.sortOrder !== undefined) out.push(["sortOrder", this.sortOrder.toString()]);
    if (this.crossFilter !== undefined) out.push(["crossFilter", this.crossFilter.toString()]);
    return out;
  }

  /** 校验所有 RequiredValidator 标注的属性都存在；缺失抛 REQUIRED_ATTR_MISSING。 */
  validateRequired(): void {
    assertRequired(this.uniqueName, { attribute: ":uniqueName", elementClass: "OlapSlicerCacheLevelData" });
    assertRequired(this.count, { attribute: ":count", elementClass: "OlapSlicerCacheLevelData" });
  }
}
