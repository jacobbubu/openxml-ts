...
// @see DocumentFormat.OpenXml.Word2010.RunConflictInsertion
...
export class RunConflictInsertion extends OpenXmlCompositeElement {
  override readonly localName = "conflictIns" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "RunConflictInsertion" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "RunConflictInsertion" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "RunConflictInsertion" });
  }
}
