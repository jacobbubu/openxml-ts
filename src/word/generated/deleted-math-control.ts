...
// @see DocumentFormat.OpenXml.Wordprocessing.DeletedMathControl
 ...
 *
 * Element: `w:del` */
export class DeletedMathControl extends OpenXmlCompositeElement {
  override readonly localName = "del" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "DeletedMathControl" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "DeletedMathControl" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "DeletedMathControl" });
  }
}
