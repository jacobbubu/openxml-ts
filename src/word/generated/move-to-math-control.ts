...
// @see DocumentFormat.OpenXml.Wordprocessing.MoveToMathControl
 ...
 *
 * Element: `w:moveTo` */
export class MoveToMathControl extends OpenXmlCompositeElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "MoveToMathControl" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "MoveToMathControl" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "MoveToMathControl" });
  }
}
