...
// @see DocumentFormat.OpenXml.Wordprocessing.Deleted
 ...
 *
 * Element: `w:del` */
export class Deleted extends OpenXmlLeafElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "Deleted" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "Deleted" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "Deleted" });
  }
}
