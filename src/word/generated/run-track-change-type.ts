...
// @see DocumentFormat.OpenXml.Wordprocessing.RunTrackChangeType
...
export abstract class RunTrackChangeType extends OpenXmlCompositeElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "RunTrackChangeType" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "RunTrackChangeType" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "RunTrackChangeType" });
  }
}
