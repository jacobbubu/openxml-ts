...
// @see DocumentFormat.OpenXml.Wordprocessing.CellDeletion
 ...
 *
 * Element: `w:cellDel` */
export class CellDeletion extends OpenXmlLeafElement {
  override readonly localName = "cellDel" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "CellDeletion" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "CellDeletion" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "CellDeletion" });
  }
}
