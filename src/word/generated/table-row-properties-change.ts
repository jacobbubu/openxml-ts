...
// @see DocumentFormat.OpenXml.Wordprocessing.TableRowPropertiesChange
 ...
 *
 * Element: `w:trPrChange` */
export class TableRowPropertiesChange extends OpenXmlCompositeElement {
  override readonly localName = "trPrChange" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "TableRowPropertiesChange" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "TableRowPropertiesChange" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "TableRowPropertiesChange" });
  }
}
