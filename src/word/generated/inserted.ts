...
// @see DocumentFormat.OpenXml.Wordprocessing.Inserted
 ...
 *
 * Element: `w:ins` */
export class Inserted extends OpenXmlLeafElement {
  override readonly localName = "ins" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "Inserted" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "Inserted" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "Inserted" });
  }
}
