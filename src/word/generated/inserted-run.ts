...
// @see DocumentFormat.OpenXml.Wordprocessing.InsertedRun
...
export class InsertedRun extends OpenXmlCompositeElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "InsertedRun" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "InsertedRun" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "InsertedRun" });
  }
}
