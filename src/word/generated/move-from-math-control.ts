...
// @see DocumentFormat.OpenXml.Wordprocessing.MoveFromMathControl
 ...
 *
 * Element: `w:moveFrom` */
export class MoveFromMathControl extends OpenXmlCompositeElement {
  override readonly localName = "moveFrom" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "MoveFromMathControl" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "MoveFromMathControl" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "MoveFromMathControl" });
  }
}
