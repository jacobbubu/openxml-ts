...
// @see DocumentFormat.OpenXml.Wordprocessing.TablePropertiesChange
 ...
 *
 * Element: `w:tblPrChange` */
export class TablePropertiesChange extends OpenXmlCompositeElement {
  override readonly localName = "tblPrChange" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      case "w:author": this.author = StringValue.parse(value); assertString(this.author, { maxLength: 255 }, { attribute: "w:author", elementClass: "TablePropertiesChange" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "TablePropertiesChange" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "TablePropertiesChange" });
  }
}
