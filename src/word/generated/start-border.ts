...
// @see DocumentFormat.OpenXml.Wordprocessing.StartBorder
 ...
 *
 * Element: `w:start` */
export class StartBorder extends OpenXmlLeafElement {
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      ...
      case "w:color": this.color = StringValue.parse(value); assertString(this.color, { maxLength: 3, minLength: 3 }, { attribute: "w:color", elementClass: "StartBorder" }); return;
      case "w:themeColor": this.themeColor = StringValue.parse(value); return;
      case "w:themeTint": this.themeTint = StringValue.parse(value); assertString(this.themeTint, { maxLength: 2, minLength: 1 }, { attribute: "w:themeTint", elementClass: "StartBorder" }); return;
      case "w:themeShade": this.themeShade = StringValue.parse(value); assertString(this.themeShade, { maxLength: 2, minLength: 1 }, { attribute: "w:themeShade", elementClass: "StartBorder" }); return;
      case "w:sz": this.size = UInt32Value.parse(value); return;
      case "w:space": this.space = UInt32Value.parse(value); assertNumber(this.space, { min: 0, max: 31 }, { attribute: "w:space", elementClass: "StartBorder" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.val, { attribute: "w:val", elementClass: "StartBorder" });
  }
}
