...
// @see DocumentFormat.OpenXml.Wordprocessing.MoveToRangeStart
 ...
 *
 * Element: `w:moveToRangeStart` */
export class MoveToRangeStart extends OpenXmlLeafElement {
  override readonly localName = "moveToRangeStart" as const;
  ...
  override applyAttribute(qname: string, value: string): void {
    switch (qname) {
      ...
      case "w:name": this.name = StringValue.parse(value); assertString(this.name, { maxLength: 40 }, { attribute: "w:name", elementClass: "MoveToRangeStart" }); return;
  ...
  validateRequired(): void {
    assertRequired(this.author, { attribute: "w:author", elementClass: "MoveToRangeStart" });
    assertRequired(this.date, { attribute: "w:date", elementClass: "MoveToRangeStart" });
    assertRequired(this.name, { attribute: "w:name", elementClass: "MoveToRangeStart" });
    assertRequired(this.id, { attribute: "w:id", elementClass: "MoveToRangeStart" });
  }
}
